...
async function sendPaymentConfirmation(data) {
  ...
  // Status badge
  const statusBadge = is_receipt
    ? `<span style="background:#d4edda;color:#155724;padding:6px 14px;border-radius:20px;font-weight:600;font-size:13px;">✓ ĐÃ XÁC MINH BIÊN LAI</span>`
    : `<span style="background:#fff3cd;color:#856404;padding:6px 14px;border-radius:20px;font-weight:600;font-size:13px;">⚠ ĐANG CHỜ XÁC MINH</span>`;

  const detectedBanksText = (detected_banks && detected_banks.length > 0)
    ? detected_banks.join(', ')
    : 'Không phát hiện';

  const noteHtml = is_receipt
    ? `<div style="background:#d4edda;color:#155724;padding:12px 16px;border-radius:8px;margin-top:16px;font-size:14px;">
         ✓ Hệ thống đã phát hiện đây là biên lai chuyển khoản hợp lệ.
         Chúng tôi đã ghi nhận và sẽ xử lý trong thời gian sớm nhất.
       </div>`
    : `<div style="background:#fff3cd;color:#856404;padding:12px 16px;border-radius:8px;margin-top:16px;font-size:14px;">
         ⚠ Hệ thống chưa thể tự động xác minh ảnh này là biên lai chuyển khoản.
         Chúng tôi sẽ kiểm tra thủ công trong vòng <strong>24 giờ</strong> và phản hồi cho bạn.
       </div>`;
...
module.exports = { sendResetCode, sendPaymentConfirmation };
